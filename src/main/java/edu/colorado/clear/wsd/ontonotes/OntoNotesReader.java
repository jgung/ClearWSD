...
public class OntoNotesReader
{
	...
	public static Set<String> getLemmas(File senseFile) throws IOException
	{
		Set<String> lemmas = new HashSet<String>();
		for (String line: FileUtils.readFileToString(senseFile).split("\n"))
		{
			ONInstance instance = new ONInstance(line);
	...
	public static List<ONInstance> readAnnotations(File senseFile, File corpusDirectory, String lemma) throws IOException
	{
		List<ONInstance> instances = new ArrayList<ONInstance>();
		for (String line: FileUtils.readFileToString(senseFile).split("\n"))
		{
			ONInstance instance = new ONInstance(line);
			if (instance.getlemma().equals(lemma))
			{
				instances.add(instance);
			}
		}

		List<ONInstance> remove = new ArrayList<ONInstance>();
		for (ONInstance i: instances)
		{
			...
			if (depFile.exists())
			{
				...
				if (!ONInstance.addToDEPTree(tree, i)) /* add sense to parse */
					remove.add(i);
			} else
				remove.add(i);
		}
