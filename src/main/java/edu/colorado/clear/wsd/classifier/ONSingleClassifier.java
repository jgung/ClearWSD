...
public class ONSingleClassifier
{
	...
	public ONSingleClassifier(
			ONClassifier parent, String lemma, boolean training) throws IOException
	{
	...
	public FeatureNode[] getFeatures(ONInstance instance)
	{
	...
	public void trainSingleClassifier(List<ONInstance> instances) throws IOException
	{
	    /* extract features */
		List<Double> yVals = new ArrayList<Double>();
		List<FeatureNode[]> features = new ArrayList<FeatureNode[]>();
		for (ONInstance instance : instances)
		{
			FeatureNode[] featuresList = getFeatures(instance);
			if (!senseToIndex.containsKey(instance.getSense()))
				senseToIndex.put(instance.getSense(), senseToIndex.size() + 1);
			int index = senseToIndex.get(instance.getSense());
	...
	public double[] getPrediction(ONInstance instance)
	{
	...
	public String classify(ONInstance instance)
	{
		FeatureNode[] features = getFeatures(instance);
		double[] probabilities = new double[senseToIndex.size()];
		Linear.predictValues(model, features, probabilities);
